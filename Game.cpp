...
bool Game::Initialize()
{
	...
	mWindow = SDL_CreateWindow("Textbook code altered by WSB", 100, 100, SCREEN_WIDTH, SCREEN_HEIGHT, 0); // WSB: changed dims
	if (!mWindow)
	{
...
void Game::LoadData()
{
	// Create player's basket; and falling head
	mBasket = new Basket(this);	mBasket->SetScale(1.5f);
	mHead   = new Head(this);	mHead  ->SetScale(1.5f);

	// Create actor for the background (this doesn't need a subclass)
	Actor* temp = new Actor(this);
	temp->SetPosition(Vector2(SCREEN_WIDTH/2, SCREEN_HEIGHT/2)); //center of screen.  WSB: updated position

	// Create the background
	BGSpriteComponent* bg = new BGSpriteComponent(temp);
	bg->SetScreenSize(Vector2(SCREEN_WIDTH, SCREEN_HEIGHT));	//WSB: updated screen dimensions
	std::vector<SDL_Texture*> bgtexs = { GetTexture("Assets/jungleWithPalms.png")}; //and set my own image
